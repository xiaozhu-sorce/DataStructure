...
void getInfo_Cart(LinkList &L)
{
    ...
    for (i = 1; i < n; i++)
    {
        ...
        {
            InitString(P);
            P.ch = (char *)malloc(StrLength(T[i]) / 3 * sizeof(char));
            Pi[j] = (char *)malloc(StrLength(T[i]) / 3 * sizeof(char));
            if (!P.ch)
                exit(OVERFLOW);
            StrCat(T2[i], P);
            StrConvert(P, Pi[j]);
